...
Errors found on the signup page was left as I could not figure out where these came from due it being a pre-defined Allauth template using django templating language and could not see any tags that hadn't been closed/where the stray tags were/any other templates that were used in conjunction with it. With more time, I would have liked to investigate these in-depth and resolve the errors, the page still works as it should.
...
| :-- | :-- | :-- | :-- | :-- | :-- |
| **VIEWING & NAVIGATION** |  |  |  |  |  |
| 1 | First Time User | Understand the concept of the website and how to use it | See what the site is about | The home page displays a clear navigation bar that has self explanatory links with text logo for the rescue, hero image, welcome header and small section of text that introduces the user to the rescue centre and explains about the rescue center | [Home Page Screenshot - Desktop](./documentation/features/home-page-screenshot.png) <br> [Home Page Screenshot - Mobile](./documentation/testing/manual_testing/screenshots/home-page-mobile.png)|
| 2 | First Time User | Go to the cat records & filter/sort them | See the cats at the rescue | The Home page features a "meet the cats" button that takes the user to the cats page when clicked, or there is a "cats" link within the navigation bar that contains a dropdown so the user can choose all cats or male/female cats. Once on the desired page, there is a button group at the top of the page which allows the user to filter by breed and colour also (it also contains the previous male/female cats options)| See above home page screenshot <br> [Cats Dropdown Menu Desktop](./documentation/testing/manual_testing/screenshots/cat-dropdown-menu.png) <br> [Cats Dropdown Menu Mobile](./documentation/testing/manual_testing/screenshots/cat-dropdown-mobile.png) <br> [Cats Sorting Buttons - Desktop](./documentation/features/filter-buttons-feedback.png) <br> [Cats Sorting Buttons - Mobile](./documentation/testing/manual_testing/screenshots/mobile-cat-sorting-buttons.png) |
| 3 | First Time User | Contact the rescue centre | Offer to volunteer or ask any questions I may have | The users can quickly find the contact link within the main navigation bar, which takes them to the contact form page. After validation and the user is able to submit the form, a success/error message is displayed | [Main Nav Links](./documentation/testing/manual_testing/screenshots/main-nav-links.png) <br> [Contact Page Screenshot](./documentation/features/contact-page-screenshot.png) <br> [Contact Form Validation](./documentation/testing/manual_testing/screenshots/contact-us-form-validation.png) <br> [Admin Email From Contact Form](./documentation/testing/manual_testing/screenshots/contact-form-email.png) |
...
| 19 | Donator | Fill in the donation form easily and quickly | Make a donation without too much hassle | The user will navigate from the main navigation bar donation link and the form is presented, auto-fill is enabled to provide ease for the user and only a few details are needed to make donation quick, there is also validation to ensure the user inputs the correct information and format (E.g. Email contains '@' symbol, the amount cannot be 0 or below) | [Donation Form Fields](./documentation/testing/manual_testing/screenshots/donation-form.png) <br> [Donation Amount Validation](./documentation/testing/manual_testing/screenshots/donation-amount-minus.png) |
...
| 6 | Had issues with a template does not exist error | Used the following stack overflow, turned out it was as simple as adding 'crispy_bootstrap5' to installed apps https://stackoverflow.com/questions/65238459/templatedoesnotexist-at-users-register-bootstrap5-uni-form-html |
...
| 8 | The javascript for the widget in the Boutique Ado walkthrough, did not work for me and my project | I customised the code by following this [tutorial](https://betterprogramming.pub/handling-file-inputs-with-javascript-9f2d3a007f05) |
| 9 | I could not get the ``` {{ MEDIA_URL }}noimage.png ``` to show up using the method in the Boutique Ado tutorial, even though it is defined in my settings and the other images in media display | Through research I found this piece of code was missing from my context processors in project level settings.py file 'django.template.context_processors.media' |
...
| 13 | Toast messages not triggering on contact form submission | After trying to google for quite some time for a resolution and not coming up with an answer, I asked chatGPT how to manually trigger a toast message. It provided me with script that I have placed in the postload js block near the bottom of the contact template, clearly commented with attribution |
| 13 | Contact page not displaying properly on mobile | Adjusted media queries for mobile, now displays properly [Mobile Contact Page Before](./documentation/testing/bugs/contact-page-mobile-not-displaying-properly.png) <br> [Mobile Contact Page After](./documentation/testing/bugs/contact-page-mobile-after.png) |
| 14 | sendEmail.js not picking up information in the contact form | Read the sendEmail.js documentation, adjusted the email template and the javascript [Contact Form Email](./documentation/testing/manual_testing/screenshots/contact-form-email.png) |
...
| :--- | :--- | :--- |
| 1 | My Account dropdown is obscured when viewed on mobile | [My Account Dropdown Menu - Mobile](./documentation/testing/bugs/mobile-dropdown-obscured.png) <br> Due to time constraints, I could not fix this issue |
| 2 | The Am I Responsive screenshot shows the mobile screen for the home page with the top element not shown in full, but it displays correctly on Safari/iPhone 14 Plus | I feel like there is a reason for this and if I had plenty of time, I could have looked into why it was doing this, but as it is displaying correctly when testing on mobile, I have not done it in this version |
| 3 | Filter buttons for breed and colour and sorting dropdown on cats page no longer has a dropdown menu/works once on the page displaying the filtered results. It only happens on the filtered pages but the other buttons still work | [Sort Buttons No Longer Displaying Dropdown](./documentation/testing/bugs/cat-sort-no-dropdown.png) Again, this was an issue of time, I would have fully looked into this with more time. |
| 4 | One cat (Fudge) picture was smaller on the card than the rest | [Fudge Cat Card](./documentation/testing/bugs/fudge-not-filling-card.png) As above, purely down to time and as it was only one cat record, I left it |
| 5 | Donation amount on emails displayed as £10.0 instead of £10.00/£10 | This is not a major issue or something that is needed to make the website work, so I did not fix it |
| 6 | Could not get the donation form to pre-fill with information in a logged in users profile | After trying several times to get this to work, I just could not figure out how to do it. |
| 7 | I couldn't style the checkbox on the edit pages, couldn't figure out why as I managed to change the one on the donation page fine | It was taking up too much time so left it in the end |