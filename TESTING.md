...
| 3 | Sorting buttons for breed and colour on cats page no longer has a dropdown menu once on the page displaying the sorted results. It only happens on the sorted pages but the other buttons still work | [Sort Buttons No Longer Displaying Dropdown](./documentation/testing/bugs/cat-sort-no-dropdown.png) Again, this was an issue of time, I would have fully looked into this with more time. |
